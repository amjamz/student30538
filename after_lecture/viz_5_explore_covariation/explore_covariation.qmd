...
```{python}
#| echo: false
movies = pd.read_json(r'C:\Users\amuly\OneDrive\Documents\GitHub\student30538\before_lecture\viz_5_explore_covariation\data\movies.json')
```
...
```{python}
alt.Chart(penguins).mark_boxplot().encode(
  alt.Y('body_mass_g:Q', scale=alt.Scale(domain=[2500, 6500])),
  alt.X('year:N')
)
```
