...
app_ui = ui.page_fluid( #always same
    [CODE TO LAY OUT THE PAGE] #we declare this code; ui side
)

def server(input, output, session): #always same
    [CODE THAT HANDLES PYTHON] 

app = App(app_ui, server) #always same; declares that we are running the Shiny app
```
:::
...
app_ui = ui.page_fluid(
    ui.panel_title("Hello Shiny!"), #header
    ui.input_slider("n", "N", 0, 100, 20), #slider variable, title of slider, min, max, auto set
    ui.output_text_verbatim("txt"), #output variable
)

def server(input, output, session):
    @render.text
    def txt():
        return f"n*2 is {input.n() * 2}" #output defined using slider variable input
